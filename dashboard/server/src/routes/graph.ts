...
router.get('/', async (_req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const nodesResult = await session.run(`
      MATCH (n)
      WHERE n:StakeholderReq OR n:SystemReq OR n:SoftwareReq OR n:TestCase OR n:InputSpec OR n:Komponente OR n:Feedback
    ...
    const edgesResult = await session.run(`
      ...
      WHERE (source:StakeholderReq OR source:SystemReq OR source:SoftwareReq OR source:TestCase OR source:InputSpec OR source:Komponente OR source:Feedback)
        AND (target:StakeholderReq OR target:SystemReq OR target:SoftwareReq OR target:TestCase OR target:InputSpec OR target:Komponente OR target:Feedback)
      RETURN
